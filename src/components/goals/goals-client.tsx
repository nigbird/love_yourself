...
import { getGoals, saveGoal, deleteGoal, completeGoal } from '../../app/goals/actions';
import { PageLayout } from "@/components/layout/page-layout";


export function GoalsClient({ initialGoals }: { initialGoals: (Goal | MeasurableGoal)[] }) {
  const [goals, setGoals] = useState<(Goal | MeasurableGoal)[]>(initialGoals);
  ...
  useEffect(() => {
    ...
    // We only fetch on subsequent renders if needed, initial data is from props
    if (!initialGoals) {
        fetchGoals();
    }
  }, [initialGoals]);
